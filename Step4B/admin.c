...
void AfficheEnteteVehiculeHV()
{
    ...
    sprintf(Tampon,"%s","Quantite");        MonPrintf(Tampon,20,strlen(Tampon));
...
void AfficheVehiculeHV(struct VehiculeHV* UnRecord)
{
    ...
    sprintf(Tampon,"%d",UnRecord->Quantite);        MonPrintf(Tampon,20,strlen(Tampon));
...
void AfficheFacture(struct FactureHV *UneFacture)
{
    ...
    timeinfo = localtime(&(UneFacture->DateFacturation));

    sprintf(Tampon,"%d",UneFacture->NumeroFacturation);     MonPrintf(Tampon,4,strlen(Tampon));
    sprintf(Tampon,"%s",UneFacture->Acheteur);              MonPrintf(Tampon,40,strlen(Tampon));
    ...
    strftime(Tampon, sizeof(Tampon), "%H:%M", timeinfo);    MonPrintf(Tampon,6,strlen(Tampon));
  ...
  while(1)
  {
    ...
    switch(Choix)
    {
        ...
        case '5':
            ...
            {
                printf("Aucun véhicule trouvé ou quantité insuffisante.\n");
            }
        ...
        case '7': AProposServeurHV("V1","Maxence","Piron");
            break;
